 ...
 $(function () {
    ...
    $(".fl").click(function(){
        $(".addImg").removeClass("icon").removeAttr("data-choose");
        $(this).find(".addImg").addClass("icon").attr("data-choose","data-choose");
    })
...
// function checkMobile(){
    //     var sMobile = document.form.phoneNum.val();
...
$("#formSave").click(function () {
    ...
    //存储input中填入的值
    // localStorage.setItem("cwx", $("#cwx").val());
    localStorage.setItem("floorNum", $("#floorNum").val());
    localStorage.setItem("addFloor", $("#addFloor").val());
    localStorage.setItem("userNum", $("#userNum").val());
    ...
    //存储radio
    var k_val = $(":radio[name=doorWidth]:checked").attr("data-value");
    localStorage.setItem("k_val", k_val);

    var j_val = $(":radio[name=sex]:checked").attr("data-value");
    localStorage.setItem("j_val", j_val);

    //存储选中的图片id
    var selectedImgId = $(".addImg[data-choose=data-choose]").attr("id");
    localStorage.setItem('imgId', selectedImgId);
})
...
$(
    function myStorage() {
        ...
        //刷新后重新获取input中填入的值
        // var cwx = localStorage.getItem("cwx");
        var floorNum = localStorage.getItem("floorNum");
        var addFloor = localStorage.getItem("addFloor");
        ...
        // $(":radio[name=doorWidth]").removeAttr("checked"); 
        $(':radio').each(function(){
            if($(this).attr("data-value") == kk_val){
                $(this).prop("checked", true);
            }    

        })

        var jj_val = localStorage.getItem("j_val");
        // $(":radio[name=doorWidth]").removeAttr("checked"); 
        $(':radio').each(function(){
            if($(this).attr("data-value") == jj_val){
                $(this).prop("checked", true);
            }    

        })

        $(":radio[value]:checked").next().css('color','#0071b9');
         

        //刷新后重新获取选中图片的id
        var imgId = localStorage.getItem("imgId");
        $("#" + imgId).addClass("icon").attr("data-choose", "data-choose");
    }
